...
function isVersionInRange(version, parsedRange) {
  ...
  for (let i = 0; i < ranges.length; i++) {
    if (isVersionInComponent(version, ranges[i])) {
      return true;
    }
  }
  return false;
...
function isVersionInComponent(version, range) {
  let minTest = true;
  let maxTest = true;
...
function intersects(r1, r2) {
  let range1 = parseRange(r1).getComponents();
  let range2 = parseRange(r2).getComponents();
  return intersectLeftRight(range1, range2) || intersectLeftRight(range2, range1);
}

function intersectLeftRight(left, right) {
  for (let i = 0; i < left.length; i++) {
    for (let j = 0; j < right.length; j++) {
      let leftComparator = left[i].maxOperand;
      let rightComparator = right[j].minOperand;

      // single element range e.g. intersect([1], [1,))
      if (!leftComparator && left[i].minOperator === '==') {
        leftComparator = left[i].minOperand;
      }
      // left range has no upper bound, use its lower bound as the representative point
      if (!leftComparator) {
        leftComparator = left[i].minOperand;
        // left's exclusive lower bound exactly touches right's upper bound - no intersection
        if (leftComparator === right[j].maxOperand && left[i].minOperator === '>') {
          continue;
        }
      }
      // left touches right but doesn't intersect e.g. intersects([0, 5),[5,))
      if (leftComparator === rightComparator && left[i].maxOperator === '<') {
        continue;
      }
      if (isVersionInComponent(leftComparator, right[j])) {
        return true;
      }
    }
  }
