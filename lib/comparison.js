...
function compareQualifiers(semver1, semver2) {
  ...
  // remove the leading dash so "sec" will be bigger than other qualifiers
  if (q1.indexOf('-sec') === 0) { q1 = q1.slice(1); }
  if (q2.indexOf('-sec') === 0) { q2 = q2.slice(1); }

  // add extra leading dash so "snapshot" will be smaller than other qualifiers
  if (q1.indexOf('-snapshot') === 0) { q1 = '-' + q1; }
  if (q2.indexOf('-snapshot') === 0) { q2 = '-' + q2; }
